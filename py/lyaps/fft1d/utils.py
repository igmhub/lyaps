...
def skyline_mask_matrices_desi(
    z_parts,
    skyline_mask_file,
    ...
    when chunks are defined with the `--parts-in-redshift` option.
    ...
    arguments lambda min, lambda max, delta lambda used to compute deltas.
    ...
    """
    skyline_list = np.genfromtxt(
        skyline_mask_file, names=("type", "wave_min", "wave_max", "frame")
    )
    ...
    for iz in range(num_parts):
        ...
        selection = (
            (skyline_list["wave_min"] <= lmax) & (skyline_list["wave_min"] >= lmin)
